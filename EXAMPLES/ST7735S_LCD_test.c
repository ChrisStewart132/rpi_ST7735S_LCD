 ...
 * Date: 06062024
 * Description: program to test basic functionality of a ST7735S 1.8" 128x160 LCD
 * 
 * gcc -o ST7735S_LCD_test ST7735S_LCD_test.c -lgpiod
 * ./ST7735S_LCD_test
 */
	...
	*/
	...
	for(int i = 0; i < HEIGHT; i++){// y
		for(int j = 0; j < WIDTH; j++){// x
			...
			uint8_t tx[2] = {b1, b2};// middle 1/3 purple
			if(i < HEIGHT/3){// top 1/3 red
				tx[1] = 0;
			} else if(i > HEIGHT - HEIGHT/3){// lower 1/3 blue
				tx[0] = 0;
			}
			
			if(j < WIDTH/3){// left 1/3 white
				tx[0] = 255;
				tx[1] = 255;
			}else if(j > WIDTH - WIDTH/3){// right 1/3 black
				tx[0] = 0;
				tx[1] = 0;
			}
